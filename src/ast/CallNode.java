...
public class CallNode implements Node {
	...
	public ArrayList<SemanticError> checkSemantics(SymbolTable ST, int _nesting)  {
		...
		if (tmp != null) {
			if(tmp.gettype() instanceof ArrowType ) {
				//controllare che tmp sia di tipo arrowtype altrimenti errore perchè chiami una variabile
				entry = tmp;
				for (Node par : parameters)
					errors.addAll(par.checkSemantics(ST, nesting));
			}else{
				System.out.println("stai trattando una variabile come una funzione");
			}
		} else {
		...
		if (_type instanceof ArrowType) {			 
			...
			if ( _partype.size() != parameters.size() ) {
				...
				for (int i = 0 ; i < parameters.size() ; i++) {
					Type par_i = (parameters.get(i)).typeCheck(t) ;
  ...
  public String codeGeneration() {
	  ...
	  for (int i=0; i < nesting - entry.getnesting() ; i++) {
	  	System.out.println("differenza "+(nesting - entry.getnesting()));
		  getAR += "store T1 0(T1) \n";
	  }
